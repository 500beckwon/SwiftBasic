...
func reorderLogFiles(_ logs: [String]) -> [String] {
    ...
    for log in logs {
        let split = log.components(separatedBy: " ")[1].first
        if split?.isNumber == true {
            digits.append(log)
        } else {
            letters.append(log)
        }
    ...
    letters.sort {
        let s1 = $0.components(separatedBy: " ")
        let s2 = $1.components(separatedBy: " ")
        
        let identity1 = s1[0]
        let identity2 = s2[0]
        
        let content1 = s1[1...].joined(separator: " ")
        let content2 = s2[1...].joined(separator: " ")
