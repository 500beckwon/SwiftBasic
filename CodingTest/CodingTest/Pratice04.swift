...
/*
 ...
 작아서 들고 다니기 편한 지갑을 만들어야 합니다. 이러한 요건을 만족하는 지갑을 만들기 위해 디자인팀은 모든 명함의 가로 길이와 세로 길이를
 조사했습니다.
...
func pratice04(_ sizes:[[Int]]) -> Int {
    var sizeList = sizes
    ...
    for i in 0..<count {
        ...
        let width = sizeList[i][0]
        let height = sizeList[i][1]
        
        widthList.append(width)
        heightList.append(height)
    }
    
    let widthMax = widthList.max() ?? 1
    let heightMax = heightList.max() ?? 1
